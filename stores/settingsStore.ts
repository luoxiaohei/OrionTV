...
interface SettingsState {
  ...
  setEnableSourceOptimization: (enabled: boolean) => Promise<void>;
...
export const useSettingsStore = create<SettingsState>((set, get) => ({
  ...
  loadSettings: async () => {
    ...
    set({
      ...
      enableSourceOptimization: settings.enableSourceOptimization ?? true,
  ...
  setEnableSourceOptimization: async (enabled) => {
    set({ enableSourceOptimization: enabled });
    await SettingsManager.save({ enableSourceOptimization: enabled });
  },
  setVideoSource: (config) => set({ videoSource: config }),
  saveSettings: async () => {
    const { apiBaseUrl, m3uUrl, remoteInputEnabled, enableSourceOptimization, videoSource } = get();
